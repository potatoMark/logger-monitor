...
public class RSACertificate {
	...
	public byte[] encodeData1(byte[] data) throws RSACertificateException {
		if (privateKey == null) {
		...
		try {
			...
			int key_len = privateKey.getModulus().bitLength() / 8 - 11;
			ByteArrayOutputStream output = new ByteArrayOutputStream();
			for (int i = 0; i < data.length; i += key_len) {
				int max = data.length - i;
				max = max > key_len ? key_len : max;
	...
	public byte[] decodeData1(byte[] data) throws RSACertificateException {
		if (publicKey == null) {
			throw new RSACertificateException("Cannot find public key!");
		}
		try {
